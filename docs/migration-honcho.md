...
## Перед шагом 3: убедись в совпадении модели эмбеддингов

Старый Honcho использовал модель для эмбеддингов (например, `BAAI/bge-m3`, `text-embedding-3-small`).
Новый стек по умолчанию использует `deepvk/USER-bge-m3`. Обе дают 1024-мерные векторы,
но пространства **разные** — поиск вернёт мусор, если модели не совпадают.

Узнай модель в исходном `.env`:
```bash
grep EMBEDDING .env  # или аналог
```

Если отличается от `deepvk/USER-bge-m3` — задай нужную в новом `.env`:
```bash
EMBEDDING_MODEL=BAAI/bge-m3   # или твоя модель
```

Все существующие векторы в дампе совместимы с той моделью, которой создавались.
Смена модели без переиндексации данных сломает семантический поиск.

Если нужно принудительно перегенерировать все эмбеддинги (например, после смены модели):
```bash
...
# Сброс без подтверждения (для скриптов)
docker compose exec honcho-api python scripts/reindex_embeddings.py --yes
```
Скрипт сбрасывает `embedding → NULL, sync_state → pending`. Reconciler подхватит строки
в течение ~5 минут и перегенерирует все векторы батчами через текущую модель.
